...
import {useGetCompanyQuery, useDeleteCompanyMutation} from "@/redux-toolkit/api/superAdmin/company.api";
...
const Companies: React.FC = () => {
  ...
  const {data, isLoading, isError, error} = useGetCompanyQuery();
 ...
 const handleConfirmDelete = async () => {
  try {
    const res = await deleteCompany(selectedCompanyId).unwrap();

    toast({
      title: "Company Deleted",
      description: res?.message || "Company deleted successfully",
    });
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-xs font-medium px-2 py-1 rounded-full ${company.active ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                            }`}
                        >
                      ...
                      <p className="text-sm text-muted-foreground">
                        Admin:- {company?.admins?.[0]?.username || "No Admin"}
                      </p>
                    </div>
