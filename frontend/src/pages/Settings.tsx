...
import { Settings as SettingsIcon, User, Bell, Lock, Eye, EyeOff, Palette, Globe, Mail, Calendar, Save, Phone, Building2, Loader2 } from 'lucide-react';
...
import { UpdateLeave } from "@/services/Service";
import { useToast } from '@/hooks/use-toast';
import { Helmet } from "react-helmet-async";
import {useGetSuperAdminQuery,useUpdateSuperAdminMutation, useSuperAdminpasswordUpateMutation} from "@/redux-toolkit/api/superAdmin/auth.api";
import {useGetByIdAdminQuery, useUpdateAdminMutation, useAdminpasswordUpateMutation} from "@/redux-toolkit/api/admin/auth.api";
...
const Settings: React.FC = () => {
  ...
  const [updateSuperAdmin, {isLoading:updateSuperAdminLoading}] = useUpdateSuperAdminMutation();
  const [updateAdmin, {isLoading:updateAdminLoading}] = useUpdateAdminMutation();
  const [superAdminpasswordUpate, {isLoading:superAdminPasswordLoading}] = useSuperAdminpasswordUpateMutation();
  const [adminpasswordUpate, {isLoading:adminPasswordLoading}] = useAdminpasswordUpateMutation();
   const {data:employeeData} = useGetEmployeeByIdQuery({id:userData?.id, companyId:userData?.companyId}, {skip: !(userData?.role === "manager" || userData?.role === "employee")});
  const {data:adminData, isLoading:adminLoading, error:adminError} = useGetByIdAdminQuery({id:userData?.id, companyId:userData?.companyId},{skip:!userData?.id || !userData?.companyId || userData?.role !== "admin"});
  const {data:superAdminData} = useGetSuperAdminQuery({id:userData?.id},{skip:!userData?.id || userData.role !== "super_admin"});
  const user = superAdminData?.data || adminData?.data || employeeData?.data;
   const {data} = useGetCompanyByIdQuery({id:userData?.companyId}, {skip:!userData?.companyId});
        ...
        const PasswordLoading = superAdminPasswordLoading || adminPasswordLoading;
  ...
  const handleUpdateLeave = async () => {
    const obj = { adminId: user?._id, companyId: user?.companyId?._id, specialLeave: leaves?.specialLeave }
    try {
  ...
  const handleSave = async () => {
    try {    
       ...
       res = await updateSuperAdmin({id:user._id, body:dataToSend}).unwrap();
        }
        else if(user?.role === "admin"){
       res = await updateAdmin({id:user._id, body:dataToSend, companyId:user?.companyId}).unwrap();
        }
         else if(user?.role === "manager" || user?.role === "employee"){
       res = await updateSuperAdmin({id:user.id, body:dataToSend}).unwrap();
        }

        toast({ title: `Profile Update Successfully.`, description: res?.message });
    } catch (err) {
      console.log(err);
      toast({ title: "Error", description: err?.data?.errors?.[0]?.message||err?.data?.message || "Something went wrong", variant:"destructive" });
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      let res;
      if(user?.role === "super_admin"){
       res = await superAdminpasswordUpate({id:user?._id, role:user?.role, password:newPassword}).unwrap();
      }
      else if(user?.role === "admin"){
       res = await adminpasswordUpate({id:user?._id,companyId:user?.companyId, role:user?.role, password:newPassword}).unwrap();
      }
      else if(user?.role === "manager" || user?.role === "employee"){
       res = await superAdminpasswordUpate({id:user?._id, role:user?.role, password:newPassword}).unwrap();
      }
      ...
      console.log(err);
      toast({ title: "Error", description: err?.data?.message|| err?.data?.error || "Something went wrong", variant:"destructive" });
    }
  }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input value={companyDetail?.contact} disabled className="pl-10" />
                  </div>
                </div>
